...
export async function tagall(client, message) {
    ...
    try {
        ...
        await client.sendMessage(remoteJid, {
            text: `鈺攢鈱� 馃殌 Digital Crew Broadcast 鈱媆n鈹俓n${text}\n鈹俓n鈺扳攢鈱� Powered by DC243 鈱塦,
...
export async function tagadmin(client, message) {
    ...
    try {
        ...
        const text = `鈺攢鈱� 馃洝锔� Digital Crew Alert 鈱媆n鈹� Admin Alert\n鈹俓n${admins.map(user => `@${user.split('@')[0]}`).join('\n')}\n鈹俓n鈺扳攢鈱� DC243 Control 鈱塦
